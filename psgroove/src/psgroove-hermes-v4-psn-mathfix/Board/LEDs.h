...
#define LED_PORT_B_2_LED (1 << 2) 
		...
		/* Inline Functions: */
			...
			{
				/** set direction */
				DDRC  |=   LED_PORT_C_6_LED ; 
DDRB  |=   LED_PORT_B_2_LED ; 

				
				/** set off leds */
				
				PORTC  &= ~LED_PORT_C_6_LED; 
PORTB  &= ~LED_PORT_B_2_LED; 
	...
	if (LEDMask & LEDS_LED2) {
			                                   ...
			                                   const uint8_t ActiveMask)
			{
					...
					if (ActiveMask & (LEDMask & LED_PORT_C_6_LED)) {
																			PORTC |=  LED_PORT_C_6_LED;
																		}
																		if((~ActiveMask) & (LEDMask & LED_PORT_C_6_LED)) {
																			PORTC &= ~LED_PORT_C_6_LED;
																		}
	if (ActiveMask & (LEDMask & LED_PORT_B_2_LED)) {
																			PORTB |=  LED_PORT_B_2_LED;
																		}
																		if((~ActiveMask) & (LEDMask & LED_PORT_B_2_LED)) {
																			PORTB &= ~LED_PORT_B_2_LED;
			...
			{
					if ((LEDMask & LED_PORT_C_6_LED) && (PORTC & LED_PORT_C_6_LED)) {
																			PORTC |=  LED_PORT_C_6_LED;
																		} else {
																			PORTC &= ~LED_PORT_C_6_LED;
																		}
	if ((LEDMask & LED_PORT_B_2_LED) && (PORTB & LED_PORT_B_2_LED)) {
																			PORTB |=  LED_PORT_B_2_LED;
																		} else {
																			PORTB &= ~LED_PORT_B_2_LED;
																		}
